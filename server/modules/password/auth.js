...
var passwordModule = {
  ...
  setup: function(req, res, next){
    ...
    var userProvidedPasswordHash = utils.hashPassword(userProvidedPassword, function(hash){
      var task = {};
      task.password = userProvidedPasswordHash;
      db.writeAuthTask(username, authName, task, function(error, authTask, user){
        next();
      });
    });
  },
...
utils.hashPassword = function(userProvidedPassword, callback){
  bcrypt.hash(userProvidedPassword, null, function(err, hash){
    callback(hash);
  });
}
